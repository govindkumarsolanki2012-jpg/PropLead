...
import { signInWithGoogle, signInWithTestAccount } from '../../services/firebaseService';
...
export const AuthFlow: React.FC<AuthFlowProps> = () => {
  ...
  const handleTestSignIn = async () => {
    ...
    try {
      ...
      console.error('[AuthFlow] Test login error:', err);
      setErrorMessage(err?.message || 'Failed to authenticate with test user account.');
    } finally {
            ...
            >
              {testLoading ? (
                <>
                  ...
                  <span>Signing in with Test User...</span>
                </>
              ) : (
                <>
                  <FlaskConical className="w-4 h-4 text-amber-700 dark:text-amber-400" />
                  <span>Test Login (Bypass Native Google Block)</span>
                </>
              )}
            </button>
            <p className="mt-1.5 text-center text-[10px] text-amber-800/80 dark:text-amber-400/80">
              Signs into Firebase with real auth & fixed UID <span className="font-mono font-semibold">hy2zTx8b...</span>. Full Firestore persistence enabled.
            </p>
          </div>
        </div>
      </div>
